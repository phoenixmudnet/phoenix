...
void trigedit_disp_types(struct descriptor_data *d)
{
   ...
   for (i = 0; i < NUM_TRIG_TYPE_FLAGS; i++)
      {
      send_to_char(d->character, "%s%2d%s) %-20.20s  %s", grn, i + 1, nrm, 
		   types[i], !(++columns % 2) ? "\r\n" : "");
	  ...
	  switch (tolower(*arg))
		 ...
		 if (OLC_VAL(d)) 
		    ...
		    if (!GET_TRIG_TYPE(OLC_TRIG(d))) 
		       {
		       send_to_char(d->character,"Invalid Trigger Type! Answer a to abort quit!\r\n");     
		       }
		    send_to_char(d->character,"Do you wish to save the changes to the trigger? (y/n): ");     
		 ...
		 send_to_char(d->character,"Name: ");
		 break;
	      case '2':
		 OLC_MODE(d) = TRIGEDIT_INTENDED;
		 send_to_char(d->character,"0: Mobiles, 1: Objects, 2: Rooms: ");
		 break;
	      case '3':
		 ...
		 send_to_char(d->character,"Numeric argument: ");
		 break;
	      case '5':
		 ...
		 if (OLC_STORAGE(d))
		    {
		    send_to_char(d->character, "%s", OLC_STORAGE(d));
	  ...
	  switch(tolower(*arg))
	     {
	      case 'y':
		 trigedit_save(d);
		 mudlogf(CMP, MAX(LVL_IMMORT, GET_INVIS_LEV(d->character)), 
			 TRUE , "OLC: %s edits trigger %d", 
		...
		/* fall through */
		 ...
		 send_to_char(d->character,"Do you wish to save the trigger? : ");
		 return;
	     }
	  break;
...
void trigedit_save(struct descriptor_data *d)
{
   ...
   FILE *trig_file;
   int zone, top;
   char *buf=get_buffer(MAX_CMD_LENGTH);
   char *bitBuf=get_buffer(MAX_INPUT_LENGTH);
   char *fname=get_buffer(MAX_INPUT_LENGTH);
   char *logbuf=get_buffer(MAX_INPUT_LENGTH);

   if ((trig_num = real_trigger(OLC_NUM(d))) != -1)
      {
      proto = trig_index[trig_num]->proto;
     ...
     /* go through the mud and replace existing triggers         */
      ...
      while (live_trig)
	 {
	 if (GET_TRIG_RNUM(live_trig) == trig_num)
	    {
	 ...
	 if (!found) 
	    ...
	    if (trig_index[i]->vnum > OLC_NUM(d)) 
	       ...
	       CREATE(new_index[trig_num], struct index_data, 1);
	       GET_TRIG_RNUM(OLC_TRIG(d)) = trig_num;
	       new_index[trig_num]->vnum = OLC_NUM(d);
	       new_index[trig_num]->number = 0; 
	       new_index[trig_num]->func = NULL;
	       CREATE(proto, struct trig_data, 1);
	       new_index[trig_num]->proto = proto;
	       trig_data_copy(proto, trig);
		  ...
		  proto->arglist = str_dup(trig->arglist);  
	       ...
	       new_index[trig_num + 1] = trig_index[trig_num];
	 ...
	 if (dsc->connected == CON_TRIGEDIT)
	    if (GET_TRIG_RNUM(OLC_TRIG(dsc)) >= trig_num)
...
release_buffer(buf);
   ...
   for (i = zone * 100; i <= top; i++) 
      {
      if ((trig_num = real_trigger(i)) != -1) 
	 {
	 trig = trig_index[trig_num]->proto;
...
release_buffer(buf);
   ...
   rename(fname, buf);
   send_to_char(d->character,"Saving Index file\r\n");
...
#endif
   send_to_char(d->character,FMT);
#undef FMT

   editscript = OLC_SCRIPT(d);
   while (editscript)
      {
      send_to_char(d->character, "     %2d) [%s%ld%s] %s%s%s", ++i, cyn,
	  ...
	  OLC_ITEM_TYPE(d))
	 send_to_char(d->character, "   %s** Mis-matched Trigger Type **%s\r\n",grn,nrm);
      else
	 send_to_char(d->character, "\r\n");
...
int dg_script_edit_parse(struct descriptor_data *d, char *arg)
{
   ...
   switch(OLC_SCRIPT_EDIT_MODE(d)) 
	  ...
	  switch(tolower(*arg))
		 ...
		 break;
	      case 'd':
		 send_to_char(d->character, "     Which entry should be deleted?  0 to abort :");
...
/* 		    trigedit_disp_menu(d); */
	  ...
	  if (real_trigger(vnum)<0)
	     {
	     send_to_char(d->character, "Invalid Trigger VNUM!\r\n"
			  "Please enter position, vnum   (ex: 1, 200):");
	     return 1;
	     }
