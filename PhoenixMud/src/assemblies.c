...
void assemblyBootAssemblies( void )
   {
   char  *szLine=get_buffer(MAX_STRING_LENGTH);
   char  *szTag=get_buffer(MAX_STRING_LENGTH);
   char  *szType=get_buffer(MAX_STRING_LENGTH);
   int  iExtract = 0;
   int  iInRoom = 0;
   int  iType = 0;
   long  lLineCount = 0;
   ...
   if( (pFile = fopen( ASSEMBLIES_FILE, "rt" )) == NULL )
      ...
      release_buffer(szType);
      release_buffer(szTag);
      release_buffer(szLine);
      return;
      }
...
void assemblySaveAssemblies( void )
   ...
   for( i = 0; i < g_lNumAssemblies; i++)
      ...
      sprinttype( pAssembly->uchAssemblyType, AssemblyTypes, szType );
...
void assemblyListToChar( struct char_data *pCharacter )
   {
   char  *szAssmType=get_buffer(MAX_INPUT_LENGTH);
   ...
   for( i = 0; i < g_lNumAssemblies; i++ )
      ...
      if( (lRnum = real_object( g_pAssemblyTable[ i ].lVnum )) < 0 )
         {
         send_to_char(pCharacter, "[%5ld] Item non-existant!\r\n", g_pAssemblyTable[i].lVnum);
...
*/
         ...
         send_to_char(pCharacter,"[%5ld] %s (%s)\r\n", g_pAssemblyTable[ i ].lVnum,
         ...
         for( j = 0; j < g_pAssemblyTable[ i ].lNumComponents; j++ )
            ...
            if( (lRnum = real_object( g_pAssemblyTable[ i ].pComponents[ j ].lVnum )) < 0 )
               {
               send_to_char(pCharacter, " %5ld: Invalid component vnum!\r\n",
...
*/
               ...
               {
               send_to_char(pCharacter, " %5ld: %-20.20s Extract=%-3.3s InRoom=%-3.3s\r\n",
                            g_pAssemblyTable[ i ].pComponents[ j ].lVnum,
