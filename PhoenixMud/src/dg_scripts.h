...
#define MTRIG_LOAD             (1 << 13)     /* Mob is loaded              */
...
#define OTRIG_TIMER            (1 << 5)	    /* timer expires               */
...
#define OTRIG_LOAD             (1 << 13)    /* object is loaded            */
...
struct trig_data {
    ...
    struct queue_event *wait_event;	/* event to pause the trigger      */
...
void bribe_mtrigger(struct char_data *ch, struct char_data *actor,
         int amount);
int wear_otrigger(struct obj_data *obj, struct char_data *actor, int iwhere);
